...
def format(level):
    ...
    # cursor.execute('SELECT question FROM level_1 WHERE id=' + str(d) +';')
    row = (cursor.fetchall())
    print(row[0][0])
    ...
    cursor.execute(
        'SELECT question, answer1, answer2, answer3, answer4, right_answer FROM level_%d WHERE id=%d;' % (level, d))
